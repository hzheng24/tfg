...
<body>
    ...
    <!-- Topbar End -->
        ...
        <div class="d-flex flex-column align-items-center justify-content-center text-center" style="min-height: 300px">
            <h1 class="font-weight-semi-bold text-uppercase mb-3">Perfil</h1>
        </div>
    </div>
    <?php
    ...
    if (isset($_SESSION['usuario'])) {
        ...
        $sql = "SELECT * FROM direcciones WHERE direccion_id = ?";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            echo "<script type='text/javascript'>alert('Error');</script>";
            echo "<script>window.location.href='$paginanom'</script>";
        ...
        mysqli_stmt_bind_param($stmt, "i", $direccion_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        if ($row = mysqli_fetch_assoc($result)) {
            $calle = $row['calle'];
    ...
    ?>
        <div class="perfil">
            <form>
                <p>Nombre: <input type="text" class="pfinput" value="<?php echo $nombre ?>" readonly></p>
                <p>Apellidos: <input type="text" class="pfinput" value="<?php echo $apellido ?>" readonly></p>
                <p>Correo electrónico: <input type="text" class="pfinput" value="<?php echo $correo ?>" readonly></p>
                <p>Calle: <input type="text" class="pfinput" value="<?php echo $calle ?>" readonly></p>
                <p>Ciudad: <input type="text" class="pfinput" value="<?php echo $ciudad ?>" readonly></p>
                <p>Código postal: <input type="text" class="pfinput" value="<?php echo $codigo_postal ?>" readonly></p>
                <p>Pais: <input type="text" class="pfinput" value="<?php echo $pais ?>" readonly></p>
            </form>
        </div>
    <?php
    } else {
